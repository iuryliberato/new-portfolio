export const PROJECTS = [
  {
    title: "Applio/ Job Application Tracker",
    desc: "I created Applio, a full-stack job-application manager with AI integration. It offers real-time CV and Cover Letter generation, job-ad scraping, auto-filled user profiles, progress statuses, notes, and secure authentication.",
    video: "../../public/videos/ApplioVideo.mp4",
    live: "https://applio-job-tracker-025135847b67.herokuapp.com/",
    git: "https://github.com/iuryliberato/applywise-frontend",
    tags: [
      "JavaScript (ESNext)",
      "React",
      "Vite",
      "CSS3",
      "Node.js",
      "Express",
      "MongoDB",
      "JWT Authentication",
      "OpenAI API",
    ],
    buttons: { liveStyle: "accent", gitStyle: "outline" }
  },
  {
    title: "Patrycja Langa Personal Website",
    desc: "I've designed and built a dual-language personal website using Next.js, React, JavaScript and Styled-Components. The website presents education, skills and services, and includes a contact form powered by EmailJS.",
    video: "../../public/videos/PatriciaVideo.mp4",
    live: "https://www.patrycjalanga.com/En",
    git: "https://github.com/iuryliberato/PatriciaWebsite",
    tags: ["HTML5", "Styled Components", "JavaScript", "React", "Next.js", "Yarn"],
    buttons: { liveStyle: "accent", gitStyle: "outline" }
  },
    {
      title: "Get'a Wave Growser Game",
      desc: "I built a browser-based crossing-game called “Get a Wave” in a weekend using HTML, CSS and JavaScript. The game features three surfers, keyboard navigation (arrow keys), hazards like speeding cars, trucks and seagulls — plus a river to avoid — and challenges you to guide all surfers safely to the beach.",
      video: "../../public/videos/GetaWaveVideo.mp4",
      live: "https://iuryliberato.github.io/Get-a-wave/",
      git: "https://github.com/iuryliberato/Get-a-wave",
      tags: ["HTML5", "CSS3", "JavaScript"],
      buttons: { liveStyle: "accent", gitStyle: "outline" }
    },
    {
      title: "EVE Events",
      desc: "This is a React front-end with app Styled-Components and a Python/Django back-end. Users can sign up/login (including Google), switch light/dark themes, search & filter events, RSVP, and create/manage their own events from their profile.",
      video: "../../public/videos/EventsVideo.mp4",
      live: "https://eve-social-events.herokuapp.com/",
      git: "https://github.com/iuryliberato/events",
      tags: ["HTML5", "React", "Python", "Django", "Styled Components", "PostgreSQL"],
      buttons: { liveStyle: "accent", gitStyle: "outline" }
    },
   
  ];
  